...
def cron_time_list(
    ...
    for month_dhm_s in month_dhm:
        ...
        #开始时间\结束时间以外的排除
        if (int(time_ymdhm[:len_start]) < int(limit_start)) or (int(time_ymdhm[:len_end]) > int(limit_end)):
            continue
...
def isdo(cron):
    ...
    """
    try:
        rs, rtime = parse_crontab_time(cron)
        if rs != 0:
            return False

        ts = get_struct_time(time.time())
        print "\nstruct time(minute hour day month week) for %d :" % time.time(), ts
        #将时间戳与crontab配置中一行时间参数对比，判断该时间戳是否在配置设定的时间范围内
        return time_match_crontab(rtime, ts)[1]
    except:
        return False
...
def main():
    ...
    print "\nstruct time(minute hour day month week) for %d :" % time_stamp, time_struct
