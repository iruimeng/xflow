...
class TaskWeb(BaseClass):
    """
    web rerun
    """

    def __init__(self):
        super(TaskWeb, self).__init__()
    ...
    def do(self):
        ...
        """
        time.sleep(5)
        for doc in self.mgdb.task_web.find({"status": 'waiting'}):

            rs, md, durl = self.__cmdRun(doc)
            if rs == 0:
                self.__update(doc.get("_id"), status="finished",
                              md=md, durl=durl, finish_time=self.now)
            else:
                self.__update(doc.get("_id"), status="failure",
                              md=md, durl=durl, finish_time=self.now)

            self.log.info('web task finished, doc:%s' % str(doc))

    def __cmdRun(self, doc):
        """
        call shell script
        """
        ndoc = self.mgdb.task_web.find_one({"_id": ObjectId(doc.get("_id"))})
        if ndoc.get("status") != "waiting":
            self.log.info('task not waiting status, doc:%s' % str(doc))
            return True
        self.__update(doc.get("_id"), status="running", start_time=self.now)

        ddate = (datetime.datetime.now() +
                 datetime.timedelta(days=-1)).strftime("%Y%m%d")
        if doc.get('qtype') != 'hql':
            rs = self.__phpCmd(doc)
            s = str(doc.get("_id"))
        else:
            hql = doc.get("hql")

            m = hashlib.md5()
            m.update(hql + self.now)
            TaskParse().mkdirLog(ddate)

            sobj = cmds.ShellCls(None)
            hobj = cmds.HiveCls(None)
            s = m.hexdigest()
            base = os.path.join(self.logPath, ddate, s)

            rslog = base + ".txt"
            errlog = base + ".err"

            rs = sobj.cmdRun(hobj.pcmd(hql) + " > " + rslog, errlog, errlog)

        c = __import__('config')
        #save file size & line num
        llog = os.path.join(self.logPath, ddate, s) + ".txt"
        if os.path.exists(llog):
            fsize = os.path.getsize(llog)
            hsize = self.__format(fsize)

            fline = os.popen('wc -l %s' % llog).read().split()[0]
            self.__update(doc.get("_id"), fstat="%s|%s" % (hsize, fline))

        #notify sendmail
        if doc.get("remark"):
            title = doc.get("remark")
        else:
            title = "web hive hql execution is done!"

        #make download url
        durl = c.logUrl + \
            base64.b64encode("%s_%s_%d" % (s, ddate, int(time.time())))
        sendto = doc.get("email").split(",")
        body = "<a href='%s'>click here to download</a><br>%s" % (
            durl, doc.get("remark"))
        bccto = self.config.get("mail_bbc")
        utils.sendmail('web hive', sendto, title, body, headers={'Content-Type': 'text/html'}, bcc=bccto)

        return rs, s, durl

    def __format(self, fsize):
        """
        human-readable
        """
        sl = ['B', 'KB', 'MB', 'GB', 'TB']
        for i in range(4):
            if fsize < 1024:
                return str("%0.2f" % fsize) + sl[i]
            fsize = fsize / 1024.0

    def __phpCmd(self, doc):
        """
        """
        bdate = doc.get("bdate")
        edate = doc.get("edate")
        ...
        oid = str(doc.get('_id'))
        if doc.get('hdp_file'):
            ufile = doc.get('hdp_file')
        else:
            ufile = 'xxxx'
        cmd = "php /home/hdp-zhushou-msdk/mg_hadoop/php/tools/reflux_cmd.php %s %s %s %s %s %s" % (
            ufile, doc.get("appkey"), bdate, edate, doc.get("qtype"), oid)
        self.log.info(cmd)

        return os.system(cmd)

    def __update(self, id, **kw):
        """ 
        update by ObjectId
        """
        if not kw:
            return False
        return self.mgdb.task_web.update({"_id": ObjectId(id)}, {"$set": kw})
